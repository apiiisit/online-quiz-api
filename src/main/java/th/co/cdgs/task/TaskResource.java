...
public class TaskResource {
    ...
    public Response create(Task task) {
        ...
        List<Task> entity = entityManager.createQuery("FROM Task t WHERE t.user.userId = :userId AND t.quiz.quizId = :quizId", Task.class)
        		.setParameter("userId", task.getUser().getUserId())
        		.setParameter("quizId", task.getQuiz().getQuizId())
        		.getResultList();
    ...
    public Response update(@PathParam("id") Integer id, Task task) {
        ...
        if (entity == null) {
            throw new WebApplicationException("task with id of " + id + " does not exist.",
                    Status.NOT_FOUND);
        }
